...
public class EasyGridBag extends GridBagLayout
{
    ...
    */
   public EasyGridBag(int num_rows, int num_columns, Container cont)
   {
      ...
      double[] row_weights = new double[num_rows];
      double[] column_weights = new double[num_columns];
      ...
      {
         columns[i] = 1;
         column_weights[i] = 1.0;
      }

      //set some instance variables in the parent class
      this.rowHeights = rows;
      this.columnWidths = columns;
      this.rowWeights = row_weights;
      this.columnWeights = column_weights;
    ...
    */
   public void fillCellCenterWithinCell(int row, int column, Component comp)
   {
    ...
    *                 Otherwise, no action is taken.
    */
   public void fillCellAlignWithinCell(int row, int column, int alignment, Component comp)
   {
      if (cont != null && row >= 1 && row <= rows && column >= 1 && column <= columns)
      {
         c.gridy = row - 1;
    ...
    *                 Otherwise, no action is taken.
    */
   public void fillCellWithRowColSpan(int row, int column, int height, int width, int fill, Component comp)
   {
      if (cont != null && row >= 1 && row <= rows && column >= 1 && column <= columns)
      {
         c.gridy = row - 1;
    ...
    *                 Otherwise, no action is taken.
    */
   public void fillCellWithOffsets(int row, int column, int top, int left, int bottom, int right, int alignment, Component comp)
   {
      if (cont != null && row >= 1 && row <= rows && column >= 1 && column <= columns)
      {
         c.gridy = row - 1;
